...
import type { TranscriptSegment } from '@/lib/services/capture-service';
...
export default function StudentApp() {
  ...
  const handleRecordingStop = useCallback(() => {
    ...
    // 使用录音期间收集的转录数据，如果没有则使用演示数据
    const finalSegments = liveSegmentsRef.current.length > 0 
      ? liveSegmentsRef.current 
      : DEMO_SEGMENTS;
    
    setSegments(finalSegments);
    setDataSource(liveSegmentsRef.current.length > 0 ? 'live' : 'demo');
